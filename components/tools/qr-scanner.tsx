...
export function QrScanner() {
  ...
  return (
    <div>
      ...
      {result && <div className="result-enter mt-7 border-t border-border/70 pt-6" aria-live="polite"><div className="flex items-center justify-between gap-3"><label htmlFor="qr-scan-result" className="text-sm font-semibold">Decoded content</label><span className="mode-local">Not opened automatically</span></div><div className="mt-2 flex gap-2"><Textarea id="qr-scan-result" className="min-h-28 break-all font-mono text-sm" value={result} readOnly spellCheck={false} /><Button className="h-11 min-h-[44px] min-w-[44px] shrink-0 px-3" variant="outline" onClick={copy} aria-label="Copy decoded text">{message === "Decoded text copied." ? <Check /> : <Copy />}</Button></div></div>}
      <FeedbackMessage className="mt-4" tone={message === "Decoded text copied." ? "success" : busy ? "neutral" : "error"}>{message || (busy ? "Scanning locally…" : "")}</FeedbackMessage>
