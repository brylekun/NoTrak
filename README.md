...
The DNS & Domain Inspector normalizes a domain or website URL locally and removes its path before any lookup. With explicit confirmation, the browser sends only the normalized domain and selected record type directly to Cloudflare's public DNS-over-HTTPS resolver. It displays A, AAAA, CNAME, MX, NS, TXT, CAA, and SOA answers, TTLs, response status, and the resolver's DNSSEC authentication flag without visiting the website.
...
The speed test intentionally sends bulk measurement traffic to Cloudflare after explicit activation. The DNS inspector sends a normalized domain and selected record type directly to Cloudflare DNS only after confirmation. Reputation checks transmit either the confirmed full URL or a locally calculated SHA-256 hash through NoTrak to configured providers. The Password Safety Checker analyzes the password locally; after a separate confirmation, its optional breach check sends only the first five characters of a SHA-1 lookup identifier directly to Have I Been Pwned. The password and full hash never leave the browser. See the in-app Privacy and Methodology pages for exact limitations and data boundaries.
